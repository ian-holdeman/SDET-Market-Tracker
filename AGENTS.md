...
- Nightly replay selects genuine scheduled run identity before exact completed attempt, including failures and unchanged commits. Keep latest outcome, job timing, browser cases and older archive independent. A persisted identity guard cannot publish results; automatic selection must not rotate the explicit archive. See docs/pipeline-replay.md.
- Visitors never trigger execution. Demonstrations use recordings or genuine recorded CI evidence, not simulated live runs. Reuse validated evidence readers before extending publication or permissions. Identify which suites a result feed actually covers; independently available job metadata cannot establish missing case outcomes. Snapshots are validated caches, not publication authorities; revalidation must honor upstream removals/expiry. The explicit private archive publication path is the only historical exception: preserve original identity/times and retry semantics, verify integrity and revocation, label it historical, and exclude it from rolling metrics.
...
- Browser tests use an isolated production server on port 3100 and mocked external APIs; do not point them at the interactive development server. Do not rebuild shared dist while browser tests are running. Finish builds before baseline checks that inspect generated output. TypeScript checking now excludes generated output; lint also syntax-checks maintained `.mjs` scripts.
- Preserve the browser server's empty temporary working directory, OS environment allowlist and shared default API interception. Scope intentional overrides to the scenario. Keep diagnostics in unique run/attempt output paths and use observable response/application settlement for race assertions.
...
- Recording endings must show the asserted final state after authentication and finite transitions settle. Preserve actual source fingerprints and attempts when replacing a clip; review decoded playback and the poster, retain unrelated recordings, and disclose fixture limits concisely.
...
For exact geometry checks after entrance animation, wait for locator actionability before measuring. A trial click can settle scrolling/motion without changing control state; retain explicit focus for keyboard tests because WebKit range inputs do not necessarily focus on click. Do not replace strict layout assertions with broad tolerances to hide an animation race.
