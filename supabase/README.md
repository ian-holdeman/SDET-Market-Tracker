...
The selected Free project is `ynopzjdajsrdjdcsmzqb` in West US (Oregon). All four reviewed migrations through `20260908000200_registration_budget` and the 88 shared catalog/curation rows are applied. Database SSL enforcement is enabled and the maintenance connection verified TLS with certificate validation. The GitHub integration **Deploy to production** is disabled: local config/fixtures must not automatically deploy to hosted data. Use `supabase/production/catalog.sql` only for an explicitly reviewed shared-data operation. It contains no identities, roles, watchlists or prices; repeated execution deliberately restores curation and is not routine deployment behavior.
