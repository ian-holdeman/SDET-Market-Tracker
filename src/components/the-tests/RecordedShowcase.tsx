    ...
    <div className="rounded-xl border border-line bg-evidence-surface overflow-hidden shadow-xl">
      <div role="tablist" aria-label="Featured tests" className="flex overflow-x-auto border-b border-line bg-surface-900/60 p-2 gap-1">
        {showcaseScenarios.map((item, index) => <button key={item.id} role="tab"
          ...
          }} className={`shrink-0 rounded-lg px-3 py-2.5 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-focus-positive ${selected === index ? 'bg-positive-500/10 text-positive-ink-300' : 'text-ink-muted hover:bg-surface-800 hover:text-ink-heading'}`}>
          {item.label}
        </button>)}
      </div>
