...
const buttonStyle = 'rounded-lg border border-line-strong px-4 py-2 text-sm font-semibold text-ink-strong hover:bg-surface-800 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-focus-positive';
  ...
  return <>
    <div className="relative aspect-video bg-black" data-testid="recording-monitor">
      ...
      {!started && !error && <div className="absolute inset-0 flex items-center justify-center bg-black/45">
        <button onClick={start} className={`${buttonStyle} bg-surface-950/90 flex items-center gap-2`}>
          <Play className="h-4 w-4" aria-hidden="true" />Play recording
        </button>
      </div>}
      {error && <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-evidence-surface px-4 text-center">
        <p role="alert" className="text-sm text-ink-secondary">Recording could not be played. Try again or choose another test.</p>
        <button className={buttonStyle} onClick={start}>Retry recording</button>
      </div>}
    </div>
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line px-5 py-3 text-xs text-ink-muted">
      ...
      <label className="flex items-center gap-2">Playback speed
        ...
        }} className="rounded border border-line-strong bg-surface-900 p-1 text-ink-strong focus-visible:outline-focus-positive">
...
export function RecordedShowcase() {
  ...
  return <section aria-labelledby="recorded-showcase-heading" data-testid="recorded-showcase" className="space-y-4">
    <div className="pb-2 border-b border-line">
      <h2 id="recorded-showcase-heading" className="text-xl font-bold text-ink-heading flex items-center gap-2"><MonitorPlay className="h-5 w-5 text-positive-ink-400" aria-hidden="true" />Test showcase</h2>
      <p className="text-xs text-ink-muted mt-1">Automated browser tests, recorded locally with test data.</p>
    </div>
    <div className="rounded-xl border border-line bg-evidence-surface overflow-hidden shadow-xl">
      <div role="tablist" aria-label="Featured tests" className="flex overflow-x-auto border-b border-line bg-surface-900/60 p-2 gap-1">
        {showcaseScenarios.map((item, index) => <button key={item.id} role="tab"
          ...
          }} className={`shrink-0 rounded-lg px-3 py-2.5 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-focus-positive ${selected === index ? 'bg-positive-500/10 light:bg-positive-surface-950/50 text-positive-ink-300' : 'text-ink-muted hover:bg-surface-800 hover:text-ink-heading'}`}>
          {item.label}
        </button>)}
      </div>
      {showcaseScenarios.map((item, index) => <div key={item.id} role="tabpanel" tabIndex={0}
        ...
        className="focus-visible:outline focus-visible:outline-2 focus-visible:outline-inset focus-visible:outline-focus-positive">
        {selected === index && <>
          {recording ? <RecordingPlayer key={`${recording.id}-${recording.sha256}`} recording={recording} label={scenario.label} /> :
            <div className="aspect-video flex flex-col items-center justify-center gap-3 px-5 text-center bg-black/30" data-testid="recording-monitor">
              <p role="status" className="text-sm text-ink-muted">{loading ? 'Loading recordings…' : failed ? 'Recordings are unavailable right now.' : 'This recording is not available yet.'}</p>
              {failed && <button className={buttonStyle} onClick={() => setRequest(n => n + 1)}>Retry loading recordings</button>}
            </div>}
          <div className="p-5 space-y-3">
            <p id={`showcase-summary-${scenario.id}`} className="text-sm text-ink-secondary">{scenario.summary}</p>
            <p className="text-xs text-ink-subtle">Curated demonstration. Current CI results appear in the dashboard above.</p>
            <details key={scenario.id} className="text-sm text-ink-muted">
              <summary className="cursor-pointer w-fit text-ink-secondary focus-visible:outline-focus-positive">About this test</summary>
              <div className="mt-4 space-y-4 break-words">
                ...
                {recording && <dl className="grid grid-cols-1 sm:grid-cols-[8rem_1fr] gap-x-4 gap-y-2 text-xs">
                  <dt>Capture outcome</dt><dd className="text-ink-strong">{recording.outcome} · {recording.durationMs / 1000}s test execution</dd>
