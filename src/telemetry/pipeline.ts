...
export const selectedPipeline = {
  ...
  workflowId: 348891072, runId: 34213740343, attempt: 1, number: 25,
  commit: '4046057df107ebda1342f435a03249ae7a656a7f',
} as const;
...
export function validatePipeline(raw: unknown, now = Date.now()): Pipeline {
  ...
  if (r.version !== 1 || Object.entries(selectedPipeline).some(([key,value]) => r[key as keyof Pipeline] !== value) ||
    r.url !== pipelineUrl || r.outcome !== 'success' || !Array.isArray(r.jobs) || r.jobs.length !== 2) throw Error('Untrusted pipeline');
  ...
  try {
    if(r.browserEvidence) {
      ...
      if(e.environment!=='github-actions' || e.runId!==String(selectedPipeline.runId) || e.runAttempt!==selectedPipeline.attempt || e.commitSha!==selectedPipeline.commit || begin<Date.parse(job.startedAt) || end>Date.parse(job.completedAt))throw Error();
  ...
  return {version:1,...selectedPipeline,url:pipelineUrl,startedAt,outcome:'success',checkedAt,expiresAt,jobs,browserEvidence};
}
