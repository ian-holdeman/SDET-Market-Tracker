...
export function usePipeline() {
  const [data,setData]=useState<Pipeline|null>(null), [loading,setLoading]=useState(true), [revision,setRevision]=useState(0);
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try {
        while(true){const part=await reader.read();if(part.done)break;bytes+=part.value.length;if(bytes>1100000)throw Error('Oversized pipeline');text+=decoder.decode(part.value,{stream:true});}
        text+=decoder.decode();
      } finally {await reader.cancel();}
      const value=validatePipeline(JSON.parse(text));
