...
const titles = (project: string) => new RegExp(scenarios.filter(s => s.project === project && (!process.env.SHOWCASE_ONLY || s.id === process.env.SHOWCASE_ONLY))
...
export default defineConfig({
  ...
  testMatch: ['auth/auth.spec.ts', 'board/chart-session.spec.ts', 'settings/settings.spec.ts', 'telemetry/history.spec.ts'],
