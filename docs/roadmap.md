...
Home, The Board, The Tests, The Logic, Privacy, Settings/application themes, and Contact are initial version complete and owner-accepted. Feature development for this version is complete; Cloud Run deployment and hosted checks are underway under the approved release. This is a feature milestone, not deployment, current CI success, security/compliance certification or production verification.
...
- The release plan was approved; hardening, resources, migrations, initial exact-commit CI, deployment and most hosted checks are complete. Final OAuth and resume/privacy release acceptance are tracked in [deployment.md](deployment.md). Do not restart feature design.
