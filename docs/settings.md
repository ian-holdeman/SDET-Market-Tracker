...
## Audit test maintenance

The September 12 audit and final Settings recording are complete and owner-accepted locally. The clear/recovery scenario ends on the authenticated empty Watchlist tab, asserting zero rows and preserved shared AAPL/MSFT curation. Recording details disclose that small mocked Board; clearing personal items does not remove shared assets. See [recording provenance](test-recordings.md).

Appearance validation is split into populated-page/dialog readability, mounted-chart preservation and decoded-video preservation in themes.spec.ts. Contrast, CSP/request origins, playback speed/color and focused-control protections remain; captures use unique per-run and per-attempt Playwright output paths. Header fit samples retain both sides of layout breakpoints in both engines/palettes. Account-race checks now establish in-flight requests before switching identity and observe response/application settlement before asserting retained state. These test changes do not alter account authority or appearance preferences.
