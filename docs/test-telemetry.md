...
The separate [nightly pipeline replay](pipeline-replay.md) reads allowlisted job timing and reuses the selected attempt's sanitized browser artifact through `/api/test-pipeline`. It does not extend browser-case artifacts or alter dashboard result semantics.
...
The complete existing Application baseline runs in two parallel jobs. `test` installs locked dependencies and browser engines, runs lint, both builds, offline checks and desktop Chromium/mobile WebKit, then attempts sanitized publication even after a test failure. `database` starts disposable runner-local Supabase, resets only that disposable database, and runs SQL tests/lint and real local Auth integration, with unconditional cleanup. Live-provider checks remain opt-in. The completed [suite audit](test-audit-implementation.md) documents F1–F14 maintenance and retained assertions across these boundaries.

Scheduled events use the existing ingestion and history/activity readers. Repository, branch, workflow, commit, run and attempt checks retain their existing boundaries; the case contract is unchanged. Distinct nightly run IDs remain distinct even at the same SHA. The strict historical archive validator and selected archive are retained; the separate nightly replay selects genuine scheduled attempts automatically. Ingestion/upload success is not suite success; the workflow conclusion preserves database failures while artifact counts remain browser-only.
...
- Original failed regressions, retries and environment/tool failures remain in private development evidence. The schedule extension did not change SQL/Auth behavior; the subsequent audit separately ran SQL/RLS and disposable Auth/PostgREST checks. No retained owner database was reset. These local checks do not establish genuine scheduler delivery.
...
The separate [nightly timeline](pipeline-replay.md) now discovers genuine scheduled Application baseline attempts without filtering by success or commit changes. Its v2 envelope keeps latest-nightly status, job evidence, browser cases and older archive independent. The original rolling metrics and publication authority are unchanged. A small private snapshot marker prevents rollback to an older observed selection; it contains identity only and cannot establish success or republish evidence. See [audit implementation](test-audit-implementation.md) for suite maintenance and retained boundary assertions.
