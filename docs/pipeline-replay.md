...
`src/telemetry/pipeline.ts` pins the repository, branch, workflow path/ID, run ID/number, attempt, and commit for the featured run. The reviewed selection is [Application baseline #30, attempt 1](https://github.com/ian-holdeman/SDET-Market-Tracker/actions/runs/34259098175/attempts/1), at commit `7b3b9f6049cb809f5fffa96c1ac8054fb89c1b18`. Both jobs began September 8, 2026 at 17:46:25 UTC; database finished 17:48:03 and test finished 17:55:44. The real window is 9m19s, with 1m38s overlap. Its sanitized artifact contains complete first-attempt browser passes. This is the accepted-feature commit before release hardening, not verification of the later release. Keep this demonstration curated rather than following every new run. Live retrieval requires source validation; the separately published archive below is the only durable exception.

`server/test-pipeline.ts` reads two GitHub REST resources: the exact run attempt and its jobs (`/actions/runs/{run_id}/attempts/{attempt}` and the corresponding `/jobs?per_page=100`). These metadata requests and the subsequent browser-artifact retrieval share a ten-second deadline. Metadata requests reject redirects and cap each response at 300 KB; artifact handling is described below. They use the existing server-only history configuration and Actions-read token. Live retrieval uses the existing artifact contract. The private archive adds explicitly separated storage/publication permissions, described below.
...
`GET /api/test-pipeline` has no write counterpart and accepts no visitor-selected source. One in-memory cache entry coalesces concurrent requests, reuses verified evidence for at most sixty seconds, and applies a fifteen-second failure cooldown. Missing configuration returns 503; invalid, deleted, inaccessible, or expired source metadata returns 502 without cached success. Responses use `Cache-Control: no-store`. A verified explicitly published archive can provide historical fallback; ordinary snapshot cache entries cannot.
...
The accepted scope is browser cases only; expanding offline/database/Auth case reporting is deferred. Their existing job summaries remain. The sanitized artifact schema is preserved. CI additionally checks the normal production build and uses pinned Node 24 actions; upload retains ZIP format and 90-day retention.
...
Live complete evidence takes precedence. If it expires or becomes unavailable, an independently valid archive is labelled **Archived historical run**, retaining its original date and identity. Its 60-second availability verification is not recent GitHub verification. It never enters rolling metrics or latest-run counts. Corrupt or inaccessible archive storage cannot suppress independently valid live job timing. Readers recheck selection generation to detect concurrent replacement/revocation.
